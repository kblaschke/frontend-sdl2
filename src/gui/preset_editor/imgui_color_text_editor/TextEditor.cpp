...
void TextEditor::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
    ...
    for (auto& r : mLanguageDefinition.mTokenRegexStrings)
        mRegexList.emplace_back(std::make_pair(std::regex(r.first, std::regex_constants::optimize), r.second));
...
std::string TextEditor::GetText(const Coordinates& aStart, const Coordinates& aEnd) const
{
    ...
    while (istart < iend || lstart < lend)
    {
        ...
        {
            result += static_cast<char>(line[istart].mChar);
            istart++;
        }
        else
        {
...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2& aPosition) const
{
    ...
    int lineNo = std::max(0, static_cast<int>(std::floor(local.y / mCharAdvance.y)));
    ...
    if (lineNo >= 0 && lineNo < static_cast<int>(mLines.size()))
    {
        ...
        while ((size_t) columnIndex < line.size())
        {
            ...
            else
            {
                ...
                int i = 0;
                while (i < 6 && d-- > 0)
                    buf[i++] = static_cast<char>(line[columnIndex++].mChar);
                buf[i] = '\0';
...
std::string TextEditor::GetWordAt(const Coordinates& aCoords) const
{
    ...
    for (auto it = istart; it < iend; ++it)
        r.push_back(static_cast<char>(mLines[aCoords.mLine][it].mChar));
...
{
    if (!mColorizerEnabled)
        return mPalette[static_cast<int>(PaletteIndex::Default)];
    if (aGlyph.mComment)
    ...
    {
        const auto ppcolor = mPalette[static_cast<int>(PaletteIndex::Preprocessor)];
        const unsigned int c0 = ((ppcolor & 0xff) + (color & 0xff)) / 2;
        const unsigned int c1 = (((ppcolor >> 8) & 0xff) + ((color >> 8) & 0xff)) / 2;
        const unsigned int c2 = (((ppcolor >> 16) & 0xff) + ((color >> 16) & 0xff)) / 2;
        const unsigned int c3 = (((ppcolor >> 24) & 0xff) + ((color >> 24) & 0xff)) / 2;
...
void TextEditor::Render()
{
    ...
    auto lineNo = static_cast<int>(std::floor(scrollY / mCharAdvance.y));
    auto globalLineMax = static_cast<int>(mLines.size());
    auto lineMax = std::max(0, std::min(static_cast<int>(mLines.size()) - 1, lineNo + static_cast<int>(std::floor((scrollY + contentSize.y) / mCharAdvance.y))));

    // Deduce mTextStart by evaluating mLines size (global lineMax) plus two spaces as text width
    char buf[16];
    snprintf(buf, 16, " %d ", globalLineMax);
    mTextStart = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, buf, nullptr, nullptr).x + static_cast<float>(mLeftMargin);
    ...
    if (!mLines.empty())
    {
        ...
        {
            ImVec2 lineStartScreenPos = ImVec2(cursorScreenPos.x, cursorScreenPos.y + static_cast<float>(lineNo) * mCharAdvance.y);
                ...
                if (focused)
                {
                    ...
                    if (elapsed > 400)
                    {
                        ...
                        if (mOverwrite && cindex < (int) line.size())
                        {
                            ...
                            {
                                char buf2[2];
                                buf2[0] = static_cast<char>(line[cindex].mChar);
                                buf2[1] = '\0';
            ...
            for (int i = 0; i < line.size();)
            {
                ...
                else
                {
                    ...
                    while (l-- > 0)
                        mLineBuffer.push_back(static_cast<char>(line[i++].mChar));
                }
                ++columnNo;
            }
    ...
    ImGui::Dummy(ImVec2((longest + 2), static_cast<float>(mLines.size()) * mCharAdvance.y));
...
void TextEditor::SetTextLines(const std::vector<std::string>& aLines)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < aLines.size(); ++i)
        {
            ...
            for (char j : aLine)
                mLines[i].emplace_back(j, PaletteIndex::Default);
        }
    }
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
    ...
    if (aChar == '\n')
    {
        ...
        line.erase(line.begin() + cindex, line.begin() + static_cast<long>(line.size()));
        SetCursorPosition(Coordinates(coord.mLine + 1, GetCharacterColumn(coord.mLine + 1, static_cast<int>(whitespaceSize))));
    ...
    else
    {
        ...
        if (e > 0)
        {
            ...
            if (mOverwrite && cindex < static_cast<int>(line.size()))
            {
                ...
                {
                    u.mRemoved += static_cast<char>(line[cindex].mChar);
    ...
    while (aAmount-- > 0)
    {
        ...
        else
        {
            ...
            if (cindex > 0)
            {
                ...
                {
                    while (cindex > 0 && IsUTFSequence(static_cast<char>(mLines[line][cindex].mChar)))
                        --cindex;
                }
            }
        }
...
void TextEditor::Backspace()
{
    ...
    else
    {
        ...
        else
        {
            ...
            auto cend = cindex + 1;
            while (cindex > 0 && IsUTFSequence(static_cast<char>(line[cindex].mChar)))
                --cindex;
...
void TextEditor::Copy()
{
    ...
    else
    {
        if (!mLines.empty())
        {
            ...
            for (auto& g : line)
                str.push_back(static_cast<char>(g.mChar));
...
std::vector<std::string> TextEditor::GetTextLines() const
{
    ...
    for (auto& line : mLines)
    {
        ...
        for (size_t i = 0; i < line.size(); ++i)
            text[i] = static_cast<char>(line[i].mChar);
...
void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
    ...
    for (int i = aFromLine; i < endLine; ++i)
    {
        ...
        {
            auto& col = line[j];
            buffer[j] = static_cast<char>(col.mChar);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!singleStartStr.empty() &&
                            ...
                            equals(singleStartStr.begin(), singleStartStr.end(), from, from + static_cast<long>(singleStartStr.size()), pred))
                        {
                            withinSingleLineComment = true;
                        }
                        else if (!withinSingleLineComment && currentIndex + startStr.size() <= line.size() &&
                                 equals(startStr.begin(), startStr.end(), from, from + static_cast<long>(startStr.size()), pred))
                        {
                        ...
                        if (currentIndex + 1 >= static_cast<int>(endStr.size()) &&
                            equals(endStr.begin(), endStr.end(), from + 1 - static_cast<long>(endStr.size()), from + 1, pred))
                        {
    ...
    for (size_t it = 0u; it < line.size() && it < colIndex;)
    {
        ...
        else
        {
            ...
            for (; i < 6 && d-- > 0 && it < (int) line.size(); i++, it++)
                tempCString[i] = static_cast<char>(line[it].mChar);
...
void TextEditor::EnsureCursorVisible()
{
    ...
    auto top = 1 + static_cast<int>(std::ceil(scrollY / mCharAdvance.y));
    auto bottom = static_cast<int>(std::ceil((scrollY + height) / mCharAdvance.y));

    auto left = static_cast<int>(std::ceil(scrollX / mCharAdvance.x));
    auto right = static_cast<int>(std::ceil((scrollX + width) / mCharAdvance.x));

    auto pos = GetActualCursorCoordinates();
    auto len = TextDistanceToLineStart(pos);

    if (pos.mLine < top)
        ImGui::SetScrollY(std::max(0.0f, static_cast<float>(pos.mLine - 1) * mCharAdvance.y));
    if (pos.mLine > bottom - 4)
        ImGui::SetScrollY(std::max(0.0f, static_cast<float>(pos.mLine + 4) * mCharAdvance.y - height));
    if (len + mTextStart < static_cast<float>(left) + 4)
        ImGui::SetScrollX(std::max(0.0f, len + mTextStart - 4));
    if (len + mTextStart > static_cast<float>(right) - 4)
...
int TextEditor::GetPageSize() const
{
    ...
    return static_cast<int>(std::floor(height / mCharAdvance.y));
}
...
static bool TokenizeMilkdropConstant(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
    const char* p = in_begin;
    ...
    // PI or PHI constant: $PI and $PHI
    if (*p == 'p' || *p == 'P' && p + 1 < in_end)
    {
        if (*(p + 1) == 'i' || *(p + 1) == 'I')
        {
            out_begin = in_begin;
        ...
        if ((*(p + 1) == 'h' || *(p + 1) == 'H') && p + 2 < in_end && (*(p + 2) == 'i' || *(p + 2) == 'I'))
        {
            out_begin = in_begin;
            out_end = p + 3;
            return true;
    ...
    // Only hex constant left to check: $XAABBCCDD
    if (*p != 'x' & *p != 'X')
    {
        return false;
    }
    ...
    while (p < in_end)
    {
        // handle end of hex constant
        static const char hexChars[] = "0123456789aAbBcCdDeEfF";

        bool digitValid = false;
        for (auto hexDigit : hexChars)
        {
            if (*p == hexDigit)
            {
                digitValid = true;
    ...
    // Only found $X, but no other valid digits -> don't recognize as constant.
    if (p == in_begin + 2)
    {
        return false;
    }
...
static bool TokenizeMilkdropNumber(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
    const char* p = in_begin;
...
static bool TokenizeMilkdropPunctuation(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
    (void) in_end;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::MilkdropExpression()
{
    ...
    if (!inited)
    {
        ...
        milkdropLangDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool {
            ...
            else if (TokenizeMilkdropConstant(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::CharLiteral;
            else if (TokenizeCStyleIdentifier(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Identifier;
            else if (TokenizeMilkdropNumber(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Number;
            else if (TokenizeCStylePunctuation(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Punctuation;
